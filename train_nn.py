...
from src.synthetic_data import generate_ood_synthetic_data, generate_sin_data, generate_clean_sin_function, generate_spiked_linear_data, generate_gap_sin_data
...
def get_approx_posterior_model(cfg, layer_priors, likelihood):
    ...
    if cfg.posterior.type == "map":
        return MAPPosterior(layer_priors=layer_priors, likelihood=likelihood,device="cuda" if torch.cuda.is_available() else "cpu", n_data=cfg.dataset.n_train)
...
def get_optimizer(cfg, model, train_dataset):
    ...
    if cfg.optimization.optimizer == "adam":
        ...
        optimizer = optim.AdamW(model.parameters(), lr=cfg.optimization.learning_rate)
    else:
...
def get_data(cfg):
    ...
    if cfg.dataset.label == "gap_sin":
        x_train, y_train = generate_gap_sin_data(n_samples=cfg.dataset.n_train, n_features=cfg.dataset.n_features, n_empty_features=cfg.dataset.n_empty_features, noise_std=cfg.dataset.noise_std, frequency=cfg.dataset.frequency, gap_min=cfg.dataset.gap_min, gap_max=cfg.dataset.gap_max)
...
def get_ood_data_loaders(cfg):
    ...
    if cfg.dataset.label == "gap_sin":
        # IID data loader (with gap)
        ood_x_iid, ood_y_iid = generate_gap_sin_data(n_samples=1000, n_features=cfg.dataset.n_features, n_empty_features=cfg.dataset.n_empty_features, noise_std=cfg.dataset.noise_std, gap_min=cfg.dataset.gap_min, gap_max=cfg.dataset.gap_max, frequency=cfg.dataset.frequency)
        ood_dataset_iid = torch.utils.data.TensorDataset(ood_x_iid, ood_y_iid)
        ood_loader_iid = torch.utils.data.DataLoader(ood_dataset_iid, batch_size=cfg.optimization.batch_size, shuffle=False)
        dataloaders.append(ood_loader_iid)
        
        # OOD data loader (without gap, continuous from -gap_max to gap_max)
        ood_x_nogap, ood_y_nogap = generate_sin_data(n_samples=1000, n_features=cfg.dataset.n_features, n_empty_features=cfg.dataset.n_empty_features, noise_std=cfg.dataset.noise_std, input_std=cfg.dataset.gap_max, frequency=cfg.dataset.frequency)
        ood_dataset_nogap = torch.utils.data.TensorDataset(ood_x_nogap, ood_y_nogap)
        ood_loader_nogap = torch.utils.data.DataLoader(ood_dataset_nogap, batch_size=cfg.optimization.batch_size, shuffle=False)
        dataloaders.append(ood_loader_nogap)
    if cfg.dataset.label == "spiked_linear":
...
def main(cfg: OmegaConf):
    ...
    wandb.init(
        ...
        mode= "offline" # "online"  # Change to "offline" if no internet
    )
