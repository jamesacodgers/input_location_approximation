...
class ColdPosteriorExactGP(ExactGP):
    ...
    def __init__(self, train_x, train_y, likelihood, temperature=1.0):
        ...
        self.temperature = torch.tensor(temperature) # Ensure it's a tensor for consistency
...
def train_exact_gp(train_x, train_y, true_hyperparams, temperature=1.0, lr=0.01, epochs=500, verbose=True):
    ...
    # Only training model.mean_module.parameters() as all others are fixed
    trainable_params = list(model.mean_module.parameters())
    ...
    for epoch in range(epochs):
        ...
        # Note: For ExactGP, the temperature is applied directly to the MLL
        # rather than being part of a custom MLL class.
...
def train_cold_posterior_gp_with_inducing(train_x, train_y, inducing_points, true_hyperparams, 
    ...
    # Only training model.mean_module.parameters() and variational parameters
    trainable_params = list(model.mean_module.parameters()) + \
                       list(model.variational_strategy.variational_distribution.parameters())
    optimizer = torch.optim.Adam(trainable_params, lr=lr)
...
def compare_inducing_positions_and_temperatures(train_x, train_y, test_x, test_y, true_hyperparams,
    ...
    exact_model, exact_likelihood, exact_losses = train_exact_gp(
    ...
    for x2_offset in x2_offsets:
        for temp in temperatures:
            ...
            results[key] = {
                ...
                'lower': lower,
                'upper': upper,
                'losses': losses, # Store losses here
                'mse': mse,
                'nll': nll,
    ...
    # Store exact GP results (including losses)
    results['exact'] = {
        'model': exact_model,
        ...
        'nll': exact_nll,
        'final_temp': 1.0,
        'losses': exact_losses # Store losses for exact GP too
    }
...
def plot_2d_posterior_comparison(train_x, train_y, test_x, vis_x, vis_grid, results):
    ...
    if n_offsets == 1:
        axes = axes.reshape(-1, 1) # Ensure axes is 2D if only one column
    if n_temps == 0 and n_offsets == 1: # Handle case with only exact GP
        axes = np.array([[axes]])
    elif n_temps > 0 and n_offsets == 1: # Handle case with sparse GPs, but one column
    ...
    for i, x2_offset in enumerate(x2_offsets):
        ...
        if i == 0:
            ...
            # For subsequent columns in the exact GP row, just show text or leave blank
            ax.set_visible(False) # Hide axis completely if not showing plot
            # Alternatively, if you want a label for the column:
            # ax.text(0.5, 0.5, f'x2={x2_offset}\n(Ref: Exact)', 
        ...
        for j, temp in enumerate(temperatures):
            ...
            # Add horizontal line at inducing point x2_offset
...
def plot_1d_comparison(train_x, train_y, test_x, test_y, results):
    ...
    n_offsets = len(x2_offsets)
    # n_temps = len(temperatures) # Not directly used for subplot grid here
    ...
    colors = ['blue', 'red', 'green', 'purple', 'orange', 'brown', 'pink'] # More colors for more temps
    
    for i, x2_offset in enumerate(x2_offsets):
        ax = axes[i]
        
        # Plot exact GP first (reference)
        ax.plot(test_x[:, 0].numpy(), test_y.numpy(), 'k--', linewidth=1.5, label='True Function', alpha=0.7)
        ax.plot(test_x[:, 0].numpy(), exact_result['pred_mean'].numpy(), 
                'k-', linewidth=2, label='Exact GP Mean', alpha=0.8)
        ax.fill_between(test_x[:, 0].numpy(), 
                       (exact_result['pred_mean'] - 1.96 * exact_result['pred_std']).numpy(),
                       (exact_result['pred_mean'] + 1.96 * exact_result['pred_std']).numpy(),
                       color='gray', alpha=0.2, label='Exact GP 95% CI')
        ...
        for j, temp in enumerate(temperatures):
            ...
            ax.plot(test_x[:, 0].numpy(), result['pred_mean'].numpy(), 
                   color=color, linewidth=1.5, linestyle='-', label=f'Sparse GP T={temp} Mean', alpha=alpha)
            
            # Plot confidence intervals
            ax.fill_between(test_x[:, 0].numpy(), 
                           result['lower'].numpy(),
                           result['upper'].numpy(),
                           color=color, alpha=0.15, label=f'Sparse GP T={temp} 95% CI') # Label only once
        
        # Plot training data
        ax.scatter(train_x[:, 0].numpy(), train_y.numpy(), 
                  c='red', s=30, marker='o', alpha=0.8, zorder=5, label='Training data')
        
        # Show inducing point positions (marked at the bottom of the plot)
        for j, temp in enumerate(temperatures):
            key = f"x2={x2_offset}_T={temp}"
            if key in sparse_results:
                result = sparse_results[key]
                inducing_points = result['inducing_points']
                # Show inducing points on x-axis with a specific marker/color
                ax.scatter(inducing_points[:, 0].numpy(), 
                          [ax.get_ylim()[0] + 0.05 * (ax.get_ylim()[1] - ax.get_ylim()[0])] * len(inducing_points), 
                          c=colors[j % len(colors)], s=40, marker='|', alpha=0.9, linewidths=2,
                          label=f'Inducing (T={temp})' if i == 0 else "_nolegend_") # Only label in first plot
        
        ax.set_xlabel('x1', fontsize=12)
        ax.set_ylabel('Function Value', fontsize=12)
        ax.set_title(f'Inducing Points x2_offset = {x2_offset}\n(Train/Test at x2=0)', fontsize=12)
        ax.grid(True, alpha=0.3)
        ax.legend(fontsize=8, loc='best') # Adjust legend position
...
def plot_1d_analysis_metrics(results):
    ...
    unique_offsets = np.unique(x2_offsets) # Not directly used for lines, but good to know
    
    # Plot 1: NLL vs x2_offset for different temperatures
    ax = axes[0]
    for temp in unique_temps:
        mask = temperatures == temp
        # Sort by x2_offset to ensure lines plot correctly
        sorted_indices = np.argsort(x2_offsets[mask])
        ax.plot(x2_offsets[mask][sorted_indices], nll_values[mask][sorted_indices], 'o-', 
    ...
    for temp in unique_temps:
        mask = temperatures == temp
        sorted_indices = np.argsort(x2_offsets[mask])
        ax.plot(x2_offsets[mask][sorted_indices], mse_values[mask][sorted_indices], 's-', 
                label=f'T={temp}', linewidth=2, markersize=8)
    ax.axhline(y=exact_mse, color='black', linestyle='--', alpha=0.7, 
               linewidth=2, label='Exact GP')
    ax.set_xlabel('Inducing Point x2 Offset')
    ax.set_ylabel('Predictive MSE')
    ax.set_title('Predictive MSE vs Inducing Position')
    ax.legend()
    ax.grid(True, alpha=0.3)
    ...
    for temp in unique_temps:
        mask = temperatures == temp
        sorted_indices = np.argsort(x2_offsets[mask])
        ax.plot(x2_offsets[mask][sorted_indices], kl_values[mask][sorted_indices], '^-', 
                label=f'T={temp}', linewidth=2, markersize=8)
    ax.set_xlabel('Inducing Point x2 Offset')
...
def plot_training_losses(results):
    """
    Plots the training losses for all models in the results dictionary.
    """
    fig, axes = plt.subplots(figsize=(12, 8))
    
    exact_losses = results['exact']['losses']
    axes.plot(exact_losses, label=f"Exact GP (T={results['exact']['final_temp']})", color='black', linestyle='--')
    
    sparse_results = {k: v for k, v in results.items() if k != 'exact'}
    
    # Get unique temperatures and offsets for consistent coloring/ordering
    all_temps = sorted(list(set([r['temperature'] for r in sparse_results.values()])))
    all_offsets = sorted(list(set([r['x2_offset'] for r in sparse_results.values()])))

    # Generate a set of distinct colors for different temperatures
    cmap = plt.get_cmap('viridis')
    temp_colors = {temp: cmap(i / (len(all_temps) - 1) if len(all_temps) > 1 else 0.5) 
                   for i, temp in enumerate(all_temps)}

    for x2_offset in all_offsets:
        for temp in all_temps:
            key = f"x2={x2_offset}_T={temp}"
            if key in sparse_results:
                losses = sparse_results[key]['losses']
                label = f"Sparse GP x2={x2_offset}, T={temp}"
                color = temp_colors[temp]
                
                # Vary linestyle based on x2_offset
                linestyle_map = {0.0: '-', 0.25: '--', 0.5: ':', 0.75: '-.', 1.0: (0, (3, 5, 1, 5))} # Custom dash for more offsets
                linestyle = linestyle_map.get(x2_offset, '-') # Default to solid if offset not mapped
                
                axes.plot(losses, label=label, color=color, linestyle=linestyle, alpha=0.8)

    axes.set_title('Training Loss Curves')
    axes.set_xlabel('Epoch')
    axes.set_ylabel('Loss (Negative MLL / Negative ELBO)')
    axes.grid(True, alpha=0.3)
    
    # Place legend outside to prevent overlap
    axes.legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize=8)
...
def run_cold_posterior_experiment():
    ...
    train_x, train_y, test_x, test_y, vis_x, vis_grid, true_hyperparams = create_synthetic_data(
        n_train=50, n_test=200, n_test_per_dim=25, noise_std=0.01  # Much lower noise
    )
    ...
    temperatures = [0.01, 0.1, 1.0, 3.0, 10.0]  # Very cold to very hot
    x2_offsets = [0.0, 0.25, 0.5, 0.75, 1.0]  # From training line to far away
    
    # Reducing epochs for quicker run during development/testing, increase for final results
    epochs = 500 
    
    results = compare_inducing_positions_and_temperatures(
        train_x, train_y, test_x, test_y, true_hyperparams,
        temperatures, x2_offsets, epochs=epochs
    )
    ...
    fig4 = plot_training_losses(results) # New plot for training losses
...
if __name__ == "__main__":
    ...
    sorted_sparse_keys = sorted(sparse_results.keys(), key=lambda k: (
        float(k.split('x2=')[1].split('_')[0]), 
        float(k.split('T=')[1])
    ))

    for key in sorted_sparse_keys:
        result = sparse_results[key]
