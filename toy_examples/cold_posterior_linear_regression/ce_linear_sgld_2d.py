...
def plot_results(X, y, w_true, mu_true, Sigma_true, models, losses_dict):
    """Plot comparison results for all models using cross entropy instead of KL divergence"""
    
    n_params = len(w_true)
    
    # Create output directory
    output_dir = "figs/cold_posterior_linear_regression_cross_entropy"
    ...
    print(f"\n{'='*60}")
    print("GENERATING TEST DATA FOR CROSS ENTROPY ANALYSIS")
    print(f"{'='*60}")
    ...
    # Compute Cross Entropies for all models
    all_test_ce = {}
    colors = {'Standard': 'red', 'Cold': 'purple', 'Tempered': 'orange'}
    
    for model_name, model in models.items():
        print(f"Computing test Cross Entropies for {model_name} VI...")
        
        test_cross_entropies = np.zeros(n_test)
        
        for i in tqdm(range(n_test), desc=f"Test CE for {model_name}"):
            ...
            # Cross entropy H(p_true, p_vi)
            cross_entropy = compute_cross_entropy(pred_mean_true, pred_var_true, 
                                                pred_mean_vi, pred_var_vi)
            test_cross_entropies[i] = cross_entropy
        
        all_test_ce[model_name] = test_cross_entropies
        
        # Print test statistics
        print(f"\n{model_name} VI Test Cross Entropy Statistics:")
    ...
    for model_name, test_ce in all_test_ce.items():
        model = models[model_name]
    ...
    axes[0].set_ylabel('H(True, VI) Predictive', fontsize=12)
    axes[0].set_title('Test Cross Entropy vs PC1', fontsize=13, fontweight='bold')
    ...
    axes[1].set_ylabel('H(True, VI) Predictive', fontsize=12)
    axes[1].set_title('Test Cross Entropy vs PC2', fontsize=13, fontweight='bold')
    axes[1].legend(fontsize=10)
    axes[1].grid(True, alpha=0.3)
    
    plt.suptitle('Cross Entropy vs Principal Components\n(Lower CE = Better Approximation)', 
                 fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    
    plt.savefig(f"{output_dir}/ce_vs_pca_components.png", dpi=300, bbox_inches='tight')
    plt.savefig(f"{output_dir}/ce_vs_pca_components.pdf", bbox_inches='tight')
    plt.show()
    ...
    for model_name, test_ce in all_test_ce.items():
        overall_stats[model_name] = {
            'mean_ce': test_ce.mean(),
            'median_ce': np.median(test_ce),
            'std_ce': test_ce.std()
        }
    
    print(f"{'Model':<15} {'Mean CE':<12} {'Median CE':<12} {'Std CE':<12}")
    print("-" * 55)
    for model_name, stats in overall_stats.items():
        print(f"{model_name:<15} {stats['mean_ce']:<12.6f} {stats['median_ce']:<12.6f} {stats['std_ce']:<12.6f}")
    
    print(f"{'='*60}")
    
    # 3. Input data visualization with Cross Entropy backgrounds
    ...
    print("Computing Cross Entropies for all models...")
    
    # First pass: compute all Cross Entropies to determine global min/max
    all_cross_entropies = []
    
    for idx, (model_name, model) in enumerate(models.items()):
        print(f"Computing Cross Entropies for {model_name} VI...")
        
        cross_entropies = np.zeros(n_grid_points)
        
        for i in tqdm(range(n_grid_points), desc=f"CE for {model_name}"):
        ...
        all_cross_entropies.append(cross_entropies)
    
    # Determine global min and max for consistent color scaling
    global_ce_min = min([ce.min() for ce in all_cross_entropies])
    global_ce_max = max([ce.max() for ce in all_cross_entropies])
    
    print(f"\nGlobal Cross Entropy range: [{global_ce_min:.6f}, {global_ce_max:.6f}]")
    
    # Second pass: create plots with consistent color scale
    for idx, (model_name, model) in enumerate(models.items()):
        cross_entropies = all_cross_entropies[idx]
        ...
        cbar.set_label('H(True, VI)', fontsize=10)
        ...
        # Print statistics
        print(f"\n{model_name} VI Cross Entropy Statistics:")
        print(f"  Mean: {cross_entropies.mean():.6f}")
        print(f"  Max:  {cross_entropies.max():.6f}")
        print(f"  Std:  {cross_entropies.std():.6f}")
    
    plt.tight_layout()
    plt.savefig(f"{output_dir}/cross_entropy_heatmaps.png", dpi=300, bbox_inches='tight')
    plt.savefig(f"{output_dir}/cross_entropy_heatmaps.pdf", bbox_inches='tight')
    print(f"Saved: {output_dir}/cross_entropy_heatmaps.png")
    print(f"Saved: {output_dir}/cross_entropy_heatmaps.pdf")
    plt.show()
    ...
    print(f"  - cross_entropy_heatmaps.png/.pdf") 
    print(f"  - ce_vs_pca_components.png/.pdf")

def main():
    """Main execution function for VI comparison using Cross Entropy"""
    print("Bayesian Linear Regression: Standard vs Cold vs Tempered VI (Cross Entropy Analysis)")
    print("="*80)
    ...
    # Plot results for all models with Cross Entropy analysis
