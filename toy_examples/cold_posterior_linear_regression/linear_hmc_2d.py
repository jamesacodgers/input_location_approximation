...
def plot_hmc_results(X, y, w_true, mu_true, Sigma_true, samplers, all_samples, log_posteriors):
    """Plot comparison results for HMC models"""
    
    n_params = len(w_true)
    
    # Create output directory
    output_dir = "figs/hmc_cold_posterior_linear_regression"
    ...
    print(f"\n{'='*60}")
    print("GENERATING TEST DATA FOR KL ANALYSIS (HMC)")
    print(f"{'='*60}")
    ...
    # Compute KL divergences for all HMC models
    all_test_kl_hmc = {}
    colors = {'Standard': 'red', 'Cold': 'purple', 'Tempered': 'orange'}
    
    for sampler_name, sampler in samplers.items():
        print(f"Computing test KL divergences for {sampler_name} HMC...")
        ...
        for i in tqdm(range(n_test), desc=f"Test KL for {sampler_name}"):
            ...
            mean_diff_sq = (pred_mean_true - pred_mean_hmc)**2
            kl_div = 0.5 * (
                np.log(pred_var_hmc / pred_var_true) +
                pred_var_true / pred_var_hmc +
                mean_diff_sq / pred_var_hmc - 1
            )
            test_kl_divergences[i] = kl_div
        
        all_test_kl_hmc[sampler_name] = test_kl_divergences
        
        # Print test statistics
        print(f"\n{sampler_name} HMC Test KL Statistics:")
    ...
    for sampler_name, test_kl in all_test_kl_hmc.items():
        sampler = samplers[sampler_name]
    ...
    axes[0].set_ylabel('KL[True || HMC] Predictive', fontsize=12)
    axes[0].set_title('Test KL Divergence vs PC1 (HMC)', fontsize=13, fontweight='bold')
    ...
    axes[1].set_ylabel('KL[True || HMC] Predictive', fontsize=12)
    axes[1].set_title('Test KL Divergence vs PC2 (HMC)', fontsize=13, fontweight='bold')
    axes[1].legend(fontsize=10)
    axes[1].grid(True, alpha=0.3)
    
    plt.suptitle('HMC: KL Divergence vs Principal Components\n(Lower KL = Better Approximation)', 
                 fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    
    plt.savefig(f"{output_dir}/hmc_kl_vs_pca_components.png", dpi=300, bbox_inches='tight')
    plt.savefig(f"{output_dir}/hmc_kl_vs_pca_components.pdf", bbox_inches='tight')
    plt.show()
    
    # 4. Input data visualization with KL divergence backgrounds (HMC version)
    ...
    print("Computing KL divergences for all HMC models...")
    
    # First pass: compute all KL divergences to determine global min/max
    all_kl_divergences_hmc = []
    
    for idx, (sampler_name, sampler) in enumerate(samplers.items()):
        print(f"Computing KL divergences for {sampler_name} HMC...")
        ...
        all_kl_divergences_hmc.append(kl_divergences)
    
    # Determine global min and max for consistent color scaling
    global_kl_min = min([kl.min() for kl in all_kl_divergences_hmc])
    global_kl_max = max([kl.max() for kl in all_kl_divergences_hmc])
    
    print(f"\nGlobal HMC KL range: [{global_kl_min:.6f}, {global_kl_max:.6f}]")
    
    # Second pass: create plots with consistent color scale
    for idx, (sampler_name, sampler) in enumerate(samplers.items()):
        kl_divergences = all_kl_divergences_hmc[idx]
        ...
        cbar.set_label('KL[True || HMC]', fontsize=10)
        ...
        # Print statistics
        print(f"\n{sampler_name} HMC KL Statistics:")
        print(f"  Mean: {kl_divergences.mean():.6f}")
        print(f"  Max:  {kl_divergences.max():.6f}")
        print(f"  Std:  {kl_divergences.std():.6f}")
    
    plt.tight_layout()
    plt.savefig(f"{output_dir}/hmc_kl_divergence_heatmaps.png", dpi=300, bbox_inches='tight')
    plt.savefig(f"{output_dir}/hmc_kl_divergence_heatmaps.pdf", bbox_inches='tight')
    print(f"Saved: {output_dir}/hmc_kl_divergence_heatmaps.png")
    plt.show()
    ...
    print(f"  - hmc_kl_vs_pca_components.png/.pdf")
    print(f"  - hmc_kl_divergence_heatmaps.png/.pdf")

def main():
    """Main execution function for HMC comparison"""
    print("Bayesian Linear Regression: HMC Standard vs Cold vs Tempered")
    print("="*70)
